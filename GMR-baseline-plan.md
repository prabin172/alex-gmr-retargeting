...
# NEXT SESSION (2026-07-18) — DECIDED: writing a paper, OURS stays core, corpus broadening in progress

**Prabin's decision (2026-07-18, supersedes the "3 options, pick one" framing below the old
version of this section): committing to writing a paper on this line. Not giving up. Contact-first
solving (OURS) stays a core mechanism — not reduced to a polish-only story.** Option 2 of the old
three (broaden to the 77-clip corpus) is now IN PROGRESS, launched this session, running in the
background (`nohup setsid ... & disown`, survives terminal logout) — see status below. Option 3
(local/adaptive over-reach correction) and the `knee_bias` regression on `fallAndGetUp1_subject1`
(new this session, see S2-T12) are both still open, not yet actioned.

Context: S2-T8/T9 DONE. S2-T10 (a fix attempt for the S2-T9 residual) tried, measured end-to-end,
REJECTED. **S2-T11 (new, 2026-07-18) found the S2-T9/T10 "pure kinematic reach limit" explanation
was incomplete** — Prabin's pushback (a too-short leg should fall short of a target, not overshoot
through the floor) caught it. Real mechanism: part genuine reach limit, part solver warm-start
local-minimum (confirmed directly — manually re-seeding the warm start resolves some of the worst
frames almost completely on the SAME target). **S2-T12 wired the fix** (`knee_bias`, ported from
the Alex pipeline, opt-in) — 4-clip result is a genuinely mixed win (2 clips clearly better incl.
the held-frame central metric, 1 clip's floorPen gets WORSE by 11cm and is not yet root-caused, 1
inert). NOT shipped as default pending that regression. Read `planLogGMR.md` `## S2-T8..T12` and
`wiki/experiments/gmr-baseline-sprint-s2.md` first -- they define the current, validated state.
The running IsaacLab GPU job is Prabin's -- do NOT stop it.

**Current validated state (4 clips: walk1_subject1, fallAndGetUp1_subject1,
fallAndGetUp2_subject2, ground1_subject1)**: OURS beats GMR+heightfix and GMR+ourpolish
decisively on held-frame support_z (the discriminating metric) on EVERY clip, including the
hardest ones -- the paper's central claim. Self-collision much improved after S2-T7's chain fix
(walk1 now matches GMR's own level). Whole-clip aggregate floorPen still trails GMR-polished
(17-40cm raw vs 1-5cm) -- PART of this is a genuine G1-vs-human kinematic reach limit (G1 is only
~64% this human's size via `root_scale`, confirmed unfixable by uniform linear scale in S2-T10),
PART is the S2-T11/T12 warm-start-basin issue (partially fixable, `knee_bias`, not yet clean).

**S3 corpus broadening — IN PROGRESS as of this write-up**: `scripts/g1/sprint_s3_full_corpus.py`
(new, resumable/skip-if-exists), plain S2-T9 config (`knee_bias` OFF, pending the fallAndGetUp1
investigation). Builds canonical_human -> Stage3 -> StageA -> ctground for all 77 LAFAN1 clips,
then evaluates GMR's 3 variants (reused from S1's existing pkls, no rebuild) + OURS's 3 variants
into one combined CSV. Launched detached from the terminal so it survives logout; check
`outputs/gmr_baseline/sprint/s3_build_nohup.log` and whether `outputs/gmr_baseline/sprint/
ours_g1_corpus/*_ctground.npz` count has reached 77 before running `--eval`.
...
- The GMR baseline column is GMR+heightfix (`outputs/gmr_baseline/sprint/pkl/{clip}_gmrfix.pkl`
  for the 77-clip corpus; the older `outputs/gmr_baseline/pkl_w2/` path was the 4-clip-only run).
- float% is a diagnostic, never a scoreboard. srcDev, never "faithfulness."
- Prabin commits git himself. Never touch `/home/ptimilsina/projects/GMR`.
- Test any scaling/weight change END-TO-END on all 4 clips before trusting an isolated-metric
  improvement (S2-T10's lesson: fixing one diagnostic number regressed everything else).
- **New (S2-T11)**: this solver is NOT bit-reproducible run-to-run even with zero code changes
  (redundant-DOF/warm-start gauge freedom) — aggregate metrics stay within ~1-2cm/1pt across
  reruns, exact qpos does not. Use metric-level comparisons, never a bit-identical qpos diff, to
  verify a no-op on `solve_lafan1_canonical_g1_contactfirst.py`.

## Next actions, roughly in order

1. Once S3's `--build` finishes all 77 clips: run `--eval`, produce the combined CSV, analyze
   (class-split table like S1's, GMR's 3 variants vs OURS's 3 variants, corpus-scale held-frame
   numbers -- the first time OURS's central claim will have full-corpus support, not just 4 clips).
2. Root-cause the `knee_bias` regression on `fallAndGetUp1_subject1` (S2-T12) -- needed before
   deciding whether `knee_bias` ships as default anywhere, including possibly re-running S3 with
   it on if it resolves cleanly.
3. Local/adaptive over-reach correction (S2-T10's untried alternative) -- still open, lower
   priority than getting full-corpus numbers first.

## Also open, not urgent

- N2/S3 (BeyondMimic): still gated on two Prabin decisions below.
- S1 Table-I->BVH mapping: still unmapped, author-contact fallback is Prabin's call.
