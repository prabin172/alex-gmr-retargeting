...
## Sprint S2: OURS on G1 (in progress, parked at CHECKPOINT M4)

The bottom row of the 2×2 — our own contact-first pipeline retargeting LAFAN1 straight to G1,
built from scratch this sprint (not an extension of E4/E4b's anchoring, which W2-T5 already ruled
out). Full trail: `planLogGMR.md` `## S2-T1..T5`.

Built and working: a LAFAN1→canonical-human adapter (reuses Stage 2.5 unchanged), a G1 Stage-3
analog (per-frame DLS IK, root+joints solved jointly, contact-held effectors) with two real bugs
found and fixed along the way (a mocap-XML body-naming footgun that silently zeroed the morphology
scale, and a degenerate hand-orientation fallback that pinned a shoulder to its limit on 61% of
frames), and — on the actual target clip (`fallAndGetUp2_subject2`, not just the walk1 sanity
check) — a pull-to-floor anchoring mechanism that, after five iterations (smoothed offset, ramp
cross-fade at zone boundaries), now produces genuinely grounded CONTACT-FRAME poses, verified by
an independent per-frame support_z audit rather than trusting the aggregate number (which had
initially, and misleadingly, looked like a win).

**Where it stands**: contact-frame quality is real and validated. Whole-clip aggregate floorPen
still trails GMR-polished (~16cm vs 4.0cm) because a meaningful fraction of a violent fall clip
has zero contact-zone signal at all (ballistic/mid-air frames) — no anchoring mechanism touches
those by construction, and pushing floor-avoidance into the solve itself is a confirmed dead end
(re-hit the exact pre-existing DLS instability `SESSION_HANDOFF.md`/`planLog.md` already
documented). This is a genuine finding, not a bug: contact-anchoring approaches — ours and the
retired E4b — are bounded by contact-detection coverage; a fall clip's flight phase needs a
different kind of grounding this kinematic pipeline doesn't have.

**CHECKPOINT M4, open**: ship the honest 5-clip OURS row as-is (contact-frame win, aggregate
honest loss on the hardest clip), try Fallback A (our contact-first targets through GMR's own mink
IK, a labeled hybrid row), or invest further in the ballistic-phase gap before broadening past the
5-clip corpus. Not decided this pass.

## Recommended next step

1. **M4 decision (Prabin)** — see above; blocks whether E7/OURS broadens to the full 77-clip
   corpus or ships as a 5-clip honest row.
