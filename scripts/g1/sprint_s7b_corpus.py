#!/usr/bin/env python3
"""S7-T3b: full 77-clip corpus build + eval for polish_median_limbwise.py's
`--center perframe` variant (S6-B2's B1b amendment). Was blocked by a divergence
bug on walk1_subject1 (t~5001-5006) -- root-caused and fixed in S7-T3 (near-
singular leg-extension configuration, `clamp_limb`'s DLS under-damped; fixed via
an opt-in `max_dq` trust region, ONLY on perframe's call path -- see
leg_floor_clamp.py's `clamp_limb` docstring for the full writeup). Gate passed
on all 5 dev clips (planLogGMR.md S7-T3), authorizing this corpus build.
Resumable (skip-if-exists), matching S6-B3's own pattern.

Usage:
    conda run -n gmr python scripts/g1/sprint_s7b_corpus.py --build
    conda run -n gmr python scripts/g1/sprint_s7b_corpus.py --eval
"""
...
OUT_CSV = REPO_ROOT / "outputs/gmr_baseline/sprint/s7b_full_corpus.csv"
PY = sys.executable
...
def do_build():
    log = REPO_ROOT / "outputs/gmr_baseline/sprint/s7b_build.log"
    ...
    for i, clip in enumerate(bvhs):
        ...
        out_pkl = PKL_S5_DIR / f"{clip}_perframelimb.pkl"
        if out_pkl.exists():
            print(f"[{i1}/{total}] SKIP (done) {clip} perframelimb")
            continue
        t0 = time.time()
        ok = run([PY, str(SCRIPTS / "polish_median_limbwise.py"),
                  ...
                  "--out", str(out_pkl),
                  "--center", "perframe", "--avoid-self-collision"], log)  # S7-T7
        print(f"[{i1}/{total}] {clip} perframelimb {'OK' if ok else 'FAIL'} ({time.time()-t0:.0f}s)")
...
def do_eval():
    ...
    for i, clip in enumerate(bvhs):
        ...
        variants = [
            ...
            ("perframelimb", PKL_S5_DIR / f"{clip}_perframelimb.pkl"),
        ]
    ...
    range_csv = REPO_ROOT / "outputs/gmr_baseline/sprint/s7b_range.csv"
