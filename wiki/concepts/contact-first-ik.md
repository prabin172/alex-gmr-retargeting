...
- **Detection** (from human markers): lowest marker within height threshold of clip floor (feet 0.07 m, hands 0.08 m; floor = 1st percentile of feet z) AND speed < 0.4 m/s. Feet additionally require a **flat** foot.
  - **Flat gate is baseline-RELATIVE (2026-07-08), not absolute.** The canonical foot frame local-Z is NOT the true sole normal — `x=toe−ankle` is the bone axis, declined ~18° (+~4° L/R skew), so a flat foot reads ~18°, not 0° (see [[orientation-frames]] FOOTGUN). So gating raw `tilt<40°` was nearly inert. Now: per-foot self-calibrated baseline = p15 of that foot's candidate-contact tilt (its flattest ≈ anatomical declination), gate `tilt − baseline < --foot-flat-margin` (default **6°**). Absolute `--foot-flat-tilt` (40°) is a fallback only when a foot has <20 candidate frames. **Corpus-validated** (detection-only diff, 20 clips): 4 real-plant frames dropped (all 1 borderline window), 3 phantom frames leaked, **0/40 feet hit the fallback**. Removes tilted/non-flat "contacts" (get-up folds, supine feet) that otherwise poison the shared `floor_z` registration — dropping ONE supine phantom cut luigi_standSupine_08 Stage-4 `floor_err` 14.3→0.68 cm. Necessary-but-not-sufficient for grounding: genuinely-flat contacts at different heights across phases remain, handled by the (planned) confidence-gated per-segment ground enforcement. See [[grounding]], [[metrics]].
